...
void pre_fill()
{
    ...
    for (int i = 0; i <= 33; i++)
    {
        ...
        printf("sh $%d,%d($%d)\n", rand_range(0, 27, true), i * 2, 0);
        printf("sb $%d,%d($%d)\n", rand_range(0, 27, true), i, 0);
    }
}
void print_instr(int x)
{
    ...
    else if (data1[x].type == 8)
    {
        y = data1[x].aux;
        printf("ori $%d,$0,%d\n", helper[y].r1, helper[y].r2);
        printf("lh $%d,%d($%d)\n", data1[x].r1, data1[x].r3, data1[x].r2);
    }
    else if (data1[x].type == 9)
    {
        y = data1[x].aux;
        printf("ori $%d,$0,%d\n", helper[y].r1, helper[y].r2);
        printf("sh $%d,%d($%d)\n", data1[x].r1, data1[x].r3, data1[x].r2);
    }
    else if (data1[x].type == 10)
    {
        y = data1[x].aux;
        printf("ori $%d,$0,%d\n", helper[y].r1, helper[y].r2);
        printf("lb $%d,%d($%d)\n", data1[x].r1, data1[x].r3, data1[x].r2);
    }
    else if (data1[x].type == 11)
    {
        y = data1[x].aux;
        printf("ori $%d,$0,%d\n", helper[y].r1, helper[y].r2);
        printf("sb $%d,%d($%d)\n", data1[x].r1, data1[x].r3, data1[x].r2);
...
{
	//根据需要更换路径 
    freopen("D:\\Archive\\ISE\\Monocycle_CPU\\Monocycle_CPU.sim\\sim_1\\behav\\xsim\\Test.asm", "w", stdout);
    pre_fill();
    ...
    {
        data1[i].type = rand_range(1, 11, false);      //
        ...
        else if(data1[i].type == 5) //sll
        {
            data1[i].r1 = rand_range(0, 27, true);
            data1[i].r2 = rand_range(0, 27, true);
            data1[i].r3 = rand_range(0, 31, false);
        }
        else if (data1[i].type == 6 || data1[i].type == 7) //sw&lw
        { 
            data1[i].aux = ++acnt;
        ...
        else if (data1[i].type == 8 || data1[i].type == 9) //sh&lh
        {
            data1[i].aux = ++acnt;
            helper[acnt].r1 = rand_range(2, 27, true);       // base reg
            helper[acnt].r2 = 4 * rand_range(0, 100, false); // imm
            ...
            data1[i].r2 = helper[acnt].r1; //base reg
            if (helper[acnt].r2 >= 50)
                data1[i].r3 = -2 * rand_range(0, 12, false);
            else
                data1[i].r3 = 2 * rand_range(0, 12, false);
        }
        else if (data1[i].type == 10|| data1[i].type == 11) //sb & sh 
        {
            data1[i].aux = ++acnt;
            helper[acnt].r1 = rand_range(2, 27, true);       // base reg
            helper[acnt].r2 = 4 * rand_range(0, 100, false); // imm
            ...
            data1[i].r2 = helper[acnt].r1; //base reg
            if (helper[acnt].r2 >= 50)
                data1[i].r3 = rand_range(0, 12, false);
            else
                data1[i].r3 = rand_range(0, 12, false);
        }
    }
